...
function extractClassTagNotAfter(text, prevChar) {
  var style = /(.*)(\{:\s*\.)(.+)(})/gm.exec(text);

  if(style && !style[1].endsWith(prevChar)) {
    return style[3];
  }
  return null;
}
...
Renderer.prototype.list = function(body, ordered) {
  ...
  var classTagStr = extractClassTagNotAfter(body, null);
...
Renderer.prototype.paragraph = function(text) {
  var classTagStr = extractClassTagNotAfter(text,">");
...
Renderer.prototype.image = function(href, title, text, raw) {
  var classTagStr = extractClassTagNotAfter(raw, null);
