...
function extractClassTag(text) {
  var style = /(\{:\s*\.)(.+)(})/g.exec(text);
  return style? style[2] : null;
}

function textWithoutTags(text) {
...
Renderer.prototype.list = function(body, ordered) {
  ...
  var classTagStr = extractClassTag(body);
...
Renderer.prototype.paragraph = function(text) {
  var classTagStr = extractClassTag(text);
  var newText = text.replace(/\\\n/g, '<br/>');

  return '<p '
      + classTag(classTagStr)
      + '>'
      + textWithoutTags(newText)
      + '</p>\n';
};
